function feature = mslProcessUNIQUE_DCT(img,W,b)
%%
...
count = 1; 
scale = 8;
%imshow(blockproc(I(:,:,1), [scale scale], @dct2))
%Convert m x n x 3 image into [(8x8x1) x count] patches
i = 1;
while (i < m - (scale - 2))
    j = 1;
    while (j< n-(scale-2)) %(j < 512)
        patch_temp = I(i:i+(scale-1),j:j+(scale-1),:);
        patch_dct = dct2(patch_temp(:,:,1));
        patches(:,count) = reshape(patch_dct,[],1);
        count = count+1;
        j = j+scale;
    end    
    i = i+scale;
